...
function installPristineApp(appName, options) {
  ...
  let setupOptions = {
    ...
    emberDataVersion: options.emberDataVersion
  };
...
function nodeModulesSetup(options) {
  ...
  let promise = Promise.resolve()
    .then(() => emberDataVersion ? addEmberDataToDependencies(appName, emberDataVersion) : null)
    .then(() => emberVersion ? updateEmberSource(appName, emberVersion) : null);

  if (!hasNodeModules) {
    if (!skipNpm) {
      promise = promise
        .then(() => runCommand('npm', 'install'));
    }
    promise = promise
...
function bowerSetup(options) {
  ...
  let promise = Promise.resolve()
    .then(() => emberVersion ? addEmberToBowerJSON(appName, emberVersion) : null)
    .then(() => removeEmberDataFromBowerJSON(appName));

  if (!hasBowerComponents) {
    promise = promise
      .then(() => runCommand('bower', 'install'))
      .then(() => debug('installed bower components'))
